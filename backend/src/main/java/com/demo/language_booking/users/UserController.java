...
public class UserController {
	...
	@PutMapping("/{id}")
	public ResponseEntity<UserPublicResponse> update(@PathVariable Long id, @RequestBody @Valid UserUpdateRequest userCreateRequest) {
	...
	public ResponseEntity<UserPublicResponse> addLanguageToUser(@PathVariable Long id, @RequestBody @Valid UserLanguageDto languageDto) {
		...
		return ResponseEntity.created(java.net.URI.create("/api/v1/users/%s/lang/%s".formatted(id,
						languageDto.getLanguage()
								.getCode())))
				.body(userMapper.mapToUserPublicResponse(updatedUser));
	}
