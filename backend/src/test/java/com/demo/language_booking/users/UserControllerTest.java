...
public class UserControllerTest {
	...
	public void createUser() throws Exception {
		...
		Mockito.verify(userService)
				.create(Mockito.any(UserCreateRequest.class));
	}
	...
	public void updateUser() throws Exception {
		...
		Mockito.when(userService.update(Mockito.anyLong(), Mockito.any(UserUpdateRequest.class)))
				.thenReturn(mockUser);
		...
		Mockito.verify(userService)
				.update(eq(1L), Mockito.any(UserUpdateRequest.class));
	}
	...
	public void updateUserWithInvalidRequestData() throws Exception {
		...
		Mockito.verify(userService, Mockito.never())
				.update(Mockito.anyLong(), Mockito.any(UserUpdateRequest.class));
	}
	...
	public void addLanguage() throws Exception {
		...
		Mockito.when(userLanguageService.addLanguage(id, userLanguageDto.getLanguage(), userLanguageDto.getLevel()))
				.thenReturn(user);
		...
		Mockito.verify(userLanguageService)
				.addLanguage(id, userLanguageDto.getLanguage(), userLanguageDto.getLevel());
	}
	...
	public void addNonSupportedLanguage() throws Exception {
		...
		mockMvc.perform(post("/api/v1/users/1/lang")
						...
						.content(objectMapper.writeValueAsString(userLanguageDto)))
				.andExpect(status().isBadRequest());
		Mockito.verify(userLanguageService, Mockito.never())
				.addLanguage(Mockito.anyLong(), Mockito.any(Language.class), Mockito.any(CEFRLevel.class));
	}
	...
	public void deleteLanguageOfNonExistentLanguage() throws Exception {
		long id = 1L;
		Language language = Language.ENGLISH;
		Mockito.doThrow(new ResourceNotFoundException("Language for user %s not found with code: %s".formatted(id,
						language.getCode())))
				.when(userLanguageService)
				.deleteLanguage(id, language);
	...
	public void updateLanguage() throws Exception {
		...
		Mockito.when(userLanguageService.updateLanguage(Mockito.anyLong(),
						Mockito.any(Language.class),
						Mockito.any(CEFRLevel.class)))
				.thenReturn(user);
		...
		Mockito.verify(userLanguageService, Mockito.atMostOnce())
				.updateLanguage(Mockito.anyLong(), Mockito.any(Language.class), Mockito.any(CEFRLevel.class));
	}
